...
# from search.models import Problem
...
def index(request):
    #recommend = Problem.objects.filter()[:5]
    return render(request, "account/index.html")

def login(request):
    # 不允许重复登录
    if request.session.get('is_login', None):
        return redirect('/dataIndex/')
...
def done_question(request):
    return render(request, "account/done_question.html")
